...
analysisname = ['batmotiv_analysis_OPIOID_all_28_3_2017.mat'];
load(analysisname);
cd(codedir);
...
stat = barcomp_motiscan_opioid(data,yname,'force duration (sec)',taskname,...
...
% yname = {'kR','kP','kE','kF','tau'};
yname = {'offset_normalizedForcePeak','mean_cond_normFPeak'};
...
%         data{isub}.battery.table.effortRatio = [];
        data{isub}.battery.table{selection,'effortRatio'} = y;
        y = tab.trialDuration -  tab.responseTime;
        data{isub}.battery.table{selection,'squeezeDuration'} = y;
end
...
% yname = 'trialDuration';
%   - transform
% ftransform = @ (x) normalize(x,'range');
ftransform = @identity;
...
figure;
[p,stat] = glm_gripAccu_motiscan_opioid(data,yname,varnames,formula,...
                                                         'plotType',plot,...
                                                         'statisticalTest',statistic);
