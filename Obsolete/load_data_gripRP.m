...
function [data] = load_data_gripRP(subjectDir, sessionList)
% LOAD_DATA_gripRP extracts data for the manip gripRP of the MBB Battery
% [data] = load_data_gripRP(subjectDir, sessionList)
%
% IN
...
%               - yankPeak             : maximal yank recorded during the trial (in N or abstract unit, depending on the device)
%       -------------------------------------------------------------------------------------------------------------
%           * calibration:
...
MANIP_NAME = '_gripRP_s'; % please keep the underscores for compatibility between versions !
...
normalizedForceSum=[];
for iSession = 1:nSession
    data.behavior.rawForceValue={data.behavior.rawForceValue{1:end} fileList(iSession).gripdata.grip{1:end}};
    data.behavior.rawTimeValue={data.behavior.rawTimeValue{1:end} fileList(iSession).gripdata.time{1:end}};
    normalizedForceSum=[normalizedForceSum data.behavior.forceSum(data.condition.sessionNumber==iSession)/data.calibration.forcePeak(iSession)*100];
end
data.behavior.normalizedForceSum=normalizedForceSum;

% for each trial, extract yankPeak
window=2;
h=ones(window,1)/window;
for i=1:length(data.behavior.rawForceValue)
    trialDuration = data.behavior.rawTimeValue{i}(end)-data.behavior.rawTimeValue{i}(1);
    dt=(trialDuration)/length(data.behavior.rawForceValue{i});
    Fdynamic_temp=filter(h,1,data.behavior.rawForceValue{i});
    Fdynamic=fliplr(filter(h,1,fliplr(Fdynamic_temp)));
    speed=diff(Fdynamic);
    Fspeed_temp=filter(h,1,speed);
    Fspeed=fliplr(filter(h,1,fliplr(Fspeed_temp)));
    data.behavior.yankPeak(i)=max(Fspeed)/dt;
end
