...
function [data] = load_data_gripP(subjectDir, sessionList)
% LOAD_DATA_gripP extracts data for the manip gripR of the MBB Battery
% [data] = load_data_gripP(subjectDir, sessionList)
%
% IN
...
MANIP_NAME = '_gripP_s'; % please keep the underscores for compatibility between versions !
...
data.condition.incentiveSign   = -1*sign(allData(:, 2)');      % 1 = trying to gain money / -1 = trying to avoid losses
