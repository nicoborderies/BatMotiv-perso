...
for i_s = 1:n_s
     ...
     if i_s<=11
        param(i_s).muV1 = mu(i_s); 
     else
         param(i_s).muV1 = 0; 
     end
     
     if i_s>11 && i_s<=22
          param(i_s).sigmaV1 = sigma(i_s-11);
     else
          param(i_s).sigmaV1 = 1;
     end
     
     if i_s>22 && i_s<=33
          param(i_s).alpha = alpha(i_s-22);
     else
          param(i_s).alpha = 1;
     end
     
     if i_s>33 && i_s<=44
          param(i_s).beta = beta(i_s-33);
     else
          param(i_s).beta = 1;
     end
     
     param(i_s).muV2 = 0; param(i_s).sigmaV2 = 1;
     param(i_s).bR = 0;
     param(i_s).bE = 0;
     param(i_s).bm = 0;
     param(i_s).kRD = 30;
...
for i_s = 1:n_s
    ...
    for j= 1:n_rep
        ...
        % sample hidden values
         param(i_s).V1 = (param(i_s).sigmaV1*randn(1,24)+param(i_s).muV1);
         param(i_s).V2 = (param(i_s).sigmaV2*randn(1,24)+param(i_s).muV2);
